        ...
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex space-x-1">
            {navItems.map((item) => (
              <motion.button
                key={item.id}
                className={`px-4 py-2 text-sm font-medium rounded-md transition-colors relative ${activeSection === item.id
            ...
            >
              <div className="flex flex-col space-y-1 py-2">
                {navItems.map((item) => (
                  <motion.button
                    key={item.id}
                    className={`px-4 py-3 text-left text-sm font-medium rounded-md transition-colors ${activeSection === item.id
                        ? 'text-white bg-blue-600/20'
                        : 'text-gray-300 hover:text-white hover:bg-white/10'
                      }`}
                    whileTap={{ scale: 0.98 }}
